...
#   - The real operator path is the in-daemon autonomous relay-claimer
#     (Step 6, [control.relay].auto_claim = true): the running node scans
#     the vault, reads the vaulted receipt, computes SignedReceipt::
#     settlement_preimage(), checks RELAY_ARMED + the margin window, and
#     submits relay_claim on its own -- single nonce owner, single vault
#     writer, no CLI.
...
# circle state before the smoke's explicit setup txs. The autonomous
# relay-claimer still targets the v4 program_addr configured below.
[chain]
...
# Step 6: the in-daemon autonomous claimer settles armed sessions itself (the
# manual `v3 relay-claim` CLI was retired). Fast scan so the smoke doesn't wait.
auto_claim            = true
claim_scan_period_secs = 5
claim_margin_epochs   = 1
...
hdr "7/ autonomous in-daemon relay-claimer"
E_BEFORE=$(view_result get_earnings_total "[\"$CIRCLE_ADDR\"]")
# Step 6: the node daemon's autonomous claimer (auto_claim=true, scan=5s) scans
# the receipt vault, and for a RELAY_ARMED session inside its margin window
# reveals SignedReceipt::settlement_preimage() and submits relay_claim ON ITS
# OWN -- no CLI, single nonce owner, single vault writer. Wait for it to drive
# the session to RELAY_CLAIMED and drain the vault entry.
wait_status "$SID" "$STATUS_RELAY_CLAIMED" "RELAY_CLAIMED (autonomous claimer)"
...
bold ""
bold "VERDICT: PASS"
say "Rust-exercised: POST /session/:id/receipt route, client settle arm I1 gate + ChainTxQueue, receipt vault, in-daemon autonomous relay-claimer."
