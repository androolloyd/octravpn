# OctraVPN v1.1 devnet harness

> **Network.** Octra runs two chains. Use **devnet** for testing:
>   * Devnet  — `https://devnet.octrascan.io/rpc` (faucet-funded; explorer at https://devnet.octrascan.io)
>   * Mainnet — `https://octra.network/rpc` (real OCT; do not use for testing)
>
> Faucet (https://faucet.octra.network/) drops devnet OCT. Epochs
> on the two chains are independent (devnet ~700k, mainnet ~857k as
> of 2026-05-15).

This directory holds the docker-compose overlay + scripts to bring up
the three-node + one-client OctraVPN harness against the Octra
**devnet**. End state: nodes register on chain, a tailnet exists, the
client opens a session, settlement completes (or you induce
equivocation and verify `slash_double_sign` works end-to-end).
...
3. **Deploy `program/main.aml`.** Three options:
   ...
   a. **CLI (recommended)** — `octra forge create` now emits a real
      ...
      The `octra` binary lives in the [`octra-foundry`][octra-foundry]
      repo (foundry-rs for Octra: forge / cast / anvil / chisel plus
      the `octraforge`, `octra-mock-rpc`, and `octra-core` libraries).
      Build it once and put the resulting binary on `$PATH`:

      ```sh
      # In the octra-foundry checkout (a sibling of this repo):
      cargo build --workspace --release
      # The binary lands at ../octra-foundry/target/release/octra.
      ```
      ...
      octra forge create program/main.aml \
        ...
        --key   $(pwd)/docker/devnet/state/deployer.key \
   ...
   c. **Pre-deployed test program.** If someone in your org already
      ...
      ```sh
      curl -s -X POST https://devnet.octrascan.io/rpc \
...
cp docker/devnet/hosts.env.example docker/devnet/hosts.env

# Edit both .env files:
$EDITOR docker/devnet/.env       # set PROGRAM_ADDR
$EDITOR docker/devnet/hosts.env  # set the four wallet addresses + public endpoints
```
...
for n in node1 node2 node3; do
  mkdir -p docker/devnet/state/$n
  cp /path/to/$n-wallet.key docker/devnet/state/$n/wallet.key
  openssl rand -hex 32 > docker/devnet/state/$n/wg.key
  chmod 600 docker/devnet/state/$n/*.key
done
mkdir -p docker/devnet/state/client
cp /path/to/client-wallet.key docker/devnet/state/client/wallet.key
chmod 600 docker/devnet/state/client/wallet.key
```
...
docker compose --profile devnet run --rm client \
  /usr/local/bin/octravpn tailnet create --treasury 1000 --acl /etc/octravpn/acl.toml --name devnet-1
...
```sh
docker compose --profile devnet exec node1 octravpn-node unbond
# wait for UNBOND_GRACE epochs…
docker compose --profile devnet exec node1 octravpn-node finalize-unbond
```
