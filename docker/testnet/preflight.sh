#!/usr/bin/env bash
# Confirm the testnet environment is ready to bring nodes up:
#   - RPC reachable
#   - Program deployed at PROGRAM_ADDR
#   - All four wallets (node1/2/3 + client) have non-zero balance
#   - Wallet + WG key files exist under $HOST_TESTNET_DIR
set -euo pipefail

cd "$(dirname "$0")/../.."
set -a
# shellcheck source=/dev/null
[[ -f docker/testnet/.env ]] && source docker/testnet/.env
[[ -f docker/testnet/hosts.env ]] && source docker/testnet/hosts.env
set +a

: "${OCTRA_RPC_URL:?set in docker/testnet/.env}"
: "${PROGRAM_ADDR:?set in docker/testnet/.env after deploy}"
...
echo
if [[ $fail -eq 0 ]]; then
  green "preflight OK — ready for: docker compose -f docker-compose.yml -f docker/testnet/docker-compose.testnet.yml --profile testnet up -d"
else
