...
6. **P0-2 autonomous claim scheduler** — *LANDED.* `relay_claimer.rs` boot-spawned actor (mirrors `run_sweeper`) owning `Arc<Hub>`, wired into `cli/runtime.rs`'s `select!`. Two ordered passes per tick over `armed_unclaimed()`: **drain** (terminal vault state only from an EXACT positive `get_session_status_strict` match — 4→`mark_claimed`, 5→`mark_refunded`; a bad/hostile read Errs and leaves the entry, never a phantom drain) then **submit** (reveal only when `epoch + margin ≤ deadline`, re-checked on a fresh pre-reveal epoch read; in-flight grace via an in-memory tick map). Routes through `hub.chain_v3` (shared `ChainTxQueue`, I4). `Hub::relay_claim_session`/`relay_confirm_and_drain` are the SOLE claim path — the CLI `v3 relay-claim` subcommand is DELETED, eliminating the second nonce owner + cross-process vault writer (residual #3). `ControlRelayCfg` gains `auto_claim` (default false), `claim_scan_period_secs` (60, clamp `[5,3600]`), `claim_margin_epochs` (3, clamp `[1,5]`), `quiescent_ticks` (1, clamp `[1,100]`). Default-off = genuine no-op. Deferred: a Hub-harness integration test (`armed→claim→drain` over a mock chain) + Prometheus counters. Note: `mark_claim_submitted` stays POST-submit (the real tx hash isn't known pre-submit and `ClaimSubmitted→ClaimSubmitted` is illegal); no-double-claim comes from the single non-overlapping actor + in-flight reservation + the on-chain status guard.
...
8. **P0-3 client refund watcher + node relay_sweep** (default-on gate) — *CLIENT REFUND WATCHER LANDED; node sweep deferred one commit.* The funder-side mirror of the Step-6 claimer: a two-pass `SettleStateStore::refund_pending` (DRAIN terminal-only from an exact positive strict status match — `RELAY_REFUNDED`→`Refunded`, `RELAY_CLAIMED`→`Settled`; then SUBMIT `relay_refund` only when still `RELAY_ARMED` and `epoch >= deadline + k_r`, write-ahead first) over the durable ladder, now extended with `RefundSubmitted(5)`/`Refunded(6)`/`Settled(7)`. Runs at client boot (folded into `replay_pending_for_client`) + a `settle refund` CLI. Client gains a strict status reader + `get_relay_deadline` + `current_epoch`, and a clamped `[v3.relay].refund_margin_epochs` (default 3, `[1,5]`). **Claim-XOR-refund proof:** on-chain `relay_claim` needs `epoch<D`, `relay_refund` needs `epoch>=D` (hard mutex, both flip status in-tx); off-chain claim only while `epoch+k_c<=D`, refund only while `epoch>=D+k_r` ⟹ overlap needs `k_c+k_r<=0`, impossible for clamped positive margins ⟹ quiet zone `(D-k_c, D+k_r)` non-empty. **Node `relay_sweep` keeper — LANDED (Step 8b).** The permissionless rescue of a permanently-offline funder: `relay_sweeper.rs`, a boot-spawned actor (parks unless `[control.relay].auto_sweep`) that — unlike the vault-scanning claimer — scans the chain session-id range `[0, get_session_count())` **cursor-bounded** (`sweep_batch` sessions/tick), and submits `relay_sweep` for any session still `RELAY_ARMED` past `deadline + get_sweep_grace() + sweep_margin` (both new AML views), through the shared `ChainTxQueue`. The sweep window is strictly LATER than both the claim (`epoch<D`) and refund (`epoch>=D+k_r`) windows, so a live operator/funder always resolves first; the keeper earns a bounty, the funder gets refunded. New: `relay_sweep_call` builder + `RELAY_SWEEP` const, node `build_relay_sweep_call` + strict `get_session_count`/`get_sweep_grace` readers, config `auto_sweep`/`sweep_scan_period_secs`(300,`[10,3600]`)/`sweep_margin_epochs`(3,`[1,20]`)/`sweep_batch`(256,`[16,4096]`). **The v4 autonomous money loop is now complete: operator claims ⟷ funder refunds ⟷ keeper sweeps, all default-off, no path able to double-drain (on-chain RELAY_ARMED mutex).**
9. **Formal models + flip default-on** — *`RelaySettlement.tla` LANDED (TLC-checked, wired into CI); Tamarin + Kani still ahead.* `proofs/tla/RelaySettlement.tla` models the v4 lifecycle (`ARMED → {CLAIMED, REFUNDED}`) with claim/refund/sweep as in-flight txs (submit / confirm / revert interleaved with epoch ticks — so a claim submitted in-window but confirming late REVERTS), and TLC exhaustively checks: **NoDoubleSettle** (claim-XOR-refund), **DrainAtMostOnce**, **Conservation**, **OnChainWindowsXOR**, **QuietZone** (I3). A negative-control config (`_NoMargin`, `Kc=Kr=0`) breaks `QuietZone` in CI, proving the margin clamp `>=1` is load-bearing; a mutex-only config shows `NoDoubleSettle` holds independent of the margins (the on-chain `RELAY_ARMED` mutex alone). Still ahead: `relay_receipt_ack.spthy` (ACK-precedes-arm, I1), Kani on the epoch-gate arithmetic. Flip defaults **only after** step 6+8 e2e + the remaining models are green.
