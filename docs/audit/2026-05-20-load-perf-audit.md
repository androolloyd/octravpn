...
| OOM-3 | Audit flusher mpsc                                    | **bounded(8192)** (Fixed)                  | Pre-fix: 125 MB/s queue growth on stall (unbounded). Post-fix: bounded queue + inline-fsync fallback; durable under stall, RAM capped at ~2 MB. Observability: `octravpn_audit_inline_fallback_total > 0` is the disk-stall signal. **Fixed in commit <pending>** | `audit.rs`; Audit-2 C-6 |
