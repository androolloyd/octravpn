...
| 4.d AEAD-impl choice for data-plane ChaCha20-Poly1305 | **Mitigated (Perf-5)** | The per-packet seal/open call sites on the data plane (`crates/octravpn-core/src/onion.rs`, `crates/octravpn-obfs4/src/frame.rs`, with WG itself still inside `boringtun`) now route through the hardware-accelerated `aws-lc-rs` shim at `crates/octravpn-core/src/aead.rs`. `aws-lc-rs` uses an assembly-tuned AVX2 / NEON implementation; on the bench, seal cost dropped from 4.48 µs to 0.93 µs (–79 %) and open from 4.46 µs to 1.09 µs (–76 %), per `docs/performance-limitations.md` §1. The portable RustCrypto crate is retained for low-throughput one-shot call sites (`crates/octravpn-core/src/stealth.rs` sealed outputs, wallet envelope under `octra-foundry`), which gain nothing from the switch. **FIPS implication:** `aws-lc-rs` is FIPS-eligible (AWS-LC is FIPS 140-3 validated under its `fips` feature); we build with `default-features = false, features = ["aws-lc-sys"]`, which enables the assembly backend but **not** the validated FIPS module. Operators that require a FIPS-validated AEAD must rebuild `octravpn-core` with `aws-lc-rs/fips` and an AWS-LC source tree configured per AWS's FIPS guide; the portable path was never FIPS-eligible. **Wire-format invariance:** the AEAD output is byte-identical to the portable path (same RFC 8439 standard); the `cross_impl_compatibility` test in `crates/octravpn-core/src/aead.rs` is the safety gate that proves it across plaintext lengths {0, 1, 16, 64, 1380, 4096}. Any divergence blocks the build. **Supply-chain:** `aws-lc-rs` and `aws-lc-sys` were already present in `Cargo.lock` transitively (via `rcgen` and `tokio-rustls`), so the switch adds no new transitive dependencies. |
