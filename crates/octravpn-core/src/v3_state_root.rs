...
fn check_hash(field: &'static str, value: &str) -> Result<(), StateRootError> {
    if value.len() != HEX_HASH_LEN {
        return Err(StateRootError::BadHashLength {
            field,
            len: value.len(),
        });
    }
    if !value.bytes().all(|b| b.is_ascii_hexdigit() && !b.is_ascii_uppercase()) {
        // Lowercase-only invariant: AML's sha256() builtin emits
        // lowercase hex; mixing cases would produce a different SHA-256
        // when the JSON is re-canonicalised, breaking the anchor.
        return Err(StateRootError::BadHashEncoding { field });
    }
    Ok(())
}
...
/// the rules in the module-level doc).
fn canonical_write(v: &Value, out: &mut Vec<u8>) {
    match v {
        ...
        Value::Object(map) => {
            ...
            // own `StateRoot` happens to declare fields in roughly the
            // right order — we cannot rely on serde's field declaration
