...
fn json_array_top_level_is_rejected() {
    ...
    assert!(resp["error"]
        .as_str()
        .unwrap()
        .contains("request must be object"));
}
...
fn keygen_with_short_seed_errors() {
    ...
    assert!(
        ...
        "expected '32 bytes' in error: {err}"
    );
}
...
fn keygen_with_odd_hex_length_errors() {
    ...
    let resp = sc.request(&json!({"op": "keygen", "seed": "012"})).unwrap();
    let err = resp["error"].as_str().unwrap();
    assert!(
...
fn premature_eof_does_not_corrupt_subsequent_run() {
    ...
    assert!(resp["sidecar"]
        .as_str()
        .unwrap()
        .starts_with("octra-pvac-sidecar/"));
}
